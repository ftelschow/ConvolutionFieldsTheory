function h = plot_errorbars( results, horzlines, xlab_name, xlims, ylab_name,...
                             ylims, LKCnum, legendplace, logplot, methods, scale, sfont, addf, ColScheme)
...
h = figure, clf, hold on;
    ...
    for n = 1:N
        ...
        for l = 1:L
           if logplot
                ErrorBar = errorbar( xxvec(l), log10(mean(results{n}.(names(l))(:,LKCnum))),...
                                     1.96 * log10(std(results{n}.(names(l))(:,LKCnum))) / ...
                                     sqrt(length( results{n}.(names(l))(:,LKCnum))), 'o' ); hold on;
           else
                ErrorBar = errorbar( xxvec(l), mean(results{n}.(names(l))(:,LKCnum)),...
                                     1.96 * std(results{n}.(names(l))(:,LKCnum)) / ...
                                     sqrt(length( results{n}.(names(l))(:,LKCnum))), 'o' ); hold on;
           end
